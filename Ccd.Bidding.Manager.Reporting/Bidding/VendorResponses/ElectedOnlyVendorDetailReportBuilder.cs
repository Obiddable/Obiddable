...
public class ElectedOnlyVendorDetailReportBuilder : BidHtmlReportBuilder
{
   ...
   protected override string Title => "Vendor Detail Report (Elected Only)";
   ...
   public ElectedOnlyVendorDetailReportBuilder() : base()
   {
   }
   ...
   public override string GenerateTableData(Bid Bid)
   {
      ...
      foreach (var vr in vendorResponses.OrderBy(x => x.VendorName).ToList())
      {
         ...
         PrintVendorResponseElectedTotalRow(t, vr);
         PrintSpacerRow(t);
      }
      PrintBidElectedTotalRow(t, vendorResponses);
      EndTable(t);
   ...
   private void PrintVendorResponseElectedTotalRow(StringBuilder t, VendorResponse vr)
   {
      t.AppendLine($"     <tr class='vendorResponseTotalRow'>");
      t.AppendLine($"         <td class='label' colspan='5'>&nbsp;</td>");
      t.AppendLine($"         <td class='quantity'>{vr.GetGetSum_ElectedQuantity(_requestingRepo).ToString("0.00")}</td>"); // yes
      t.AppendLine($"         <td class='unitPrice'>&nbsp;</td>");
      t.AppendLine($"         <td class='extensionPrice'>{vr.GetGetSum_ElectedTotalPrice(_requestingRepo).ToString("0.00")}</td>"); //yes
      t.AppendLine($"         <td class='vendorCode'>&nbsp;</td>");
      t.AppendLine($"         <td class='altDescription'>&nbsp;</td>");
      t.AppendLine($"     </tr>");
   }

   private void PrintBidElectedTotalRow(StringBuilder t, List<VendorResponse> vendorResponses)
   {
      t.AppendLine($"     <tr class='vendorResponseTotalRow'>");
      t.AppendLine($"         <td class='label' colspan='5'>&nbsp;</td>");
      t.AppendLine($"         <td class='quantity'>{vendorResponses.Sum(x => x.GetGetSum_ElectedQuantity(_requestingRepo)).ToString("0.00")}</td>"); // yes
      t.AppendLine($"         <td class='unitPrice'>&nbsp;</td>");
      t.AppendLine($"         <td class='extensionPrice'>{vendorResponses.Sum(x => x.GetGetSum_ElectedTotalPrice(_requestingRepo)).ToString("0.00")}</td>"); //yes
      t.AppendLine($"         <td class='vendorCode'>&nbsp;</td>");
      t.AppendLine($"         <td class='altDescription'>&nbsp;</td>");
      t.AppendLine($"     </tr>");
   }
