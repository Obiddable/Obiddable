...
public class SummaryReportBuilder : BidHtmlReportBuilder
{
    ...
    public SummaryReportBuilder()
    {
        ...
        _distributionService = new DistributionService(
            new EFRequestingRepo(),
            new EFLegacyElectionsRepo(),
            new EFDistributionRepo(),
    ...
    public override string GenerateTableData(Bid bid)
    {
        ...
        List<VendorResponse> vendorResponses = _respondingRepo
            .GetVendorResponses_ByBid(bid.Id)
            .OrderBy(x => x.VendorName)
            .ToList();
        foreach (var v in vendorResponses)
        {
            int vendorTotal_responsesCount = 0,
                vendorTotal_rowCount = 0;
            decimal vendorTotal_extensionPrice = 0;

            StringBuilder vRow = new StringBuilder();

            string[] buildingNames = _distributionService
                .GetRequestorsBuildingNames_Elected_ByVendorResponse(v.Id)
                .OrderBy(x => x)
                .ToArray();
            ...
            for (int x = 0; x < buildingNames.Length; x++)
            {
                ...
                int buildingTotal_responseCount = 0;
                decimal buildingTotal_extensionPrice = 0;

                List<ResponseItem> responseItems = _distributionService
                    .GetResponseItems_ByBuildingName_ByVendorResponse(b, v.Id)
                    .OrderBy(q => q.Item.Code)
                    .ToList();
                foreach (var ri in responseItems)
                {
                    ...
                    bRow.AppendLine(
                        $"  <td class='quantity'>{requestedQuantity.ToString("0.00")}</td>"
                    );
                ...
                bRow.AppendLine(
                    $"<tr class='{(x == buildingNames.Length - 1 ? "buildingSummaryRow-last" : "")} buildingSummaryRow'>"
                );
                bRow.AppendLine(
                    $"  <td class='buildingSummaryRow-label' colspan='3'>Building Total:</td>"
                );
                bRow.AppendLine(
                    $"  <td class='count' colspan='1'>{buildingTotal_responseCount.ToString("0")}</td>"
                );
                bRow.AppendLine(
                    $"  <td class='extension' colspan='1'>{buildingTotal_extensionPrice.ToString("0.00")}</td>"
                );
            ...
            vRow.AppendLine(
                $"  <td class='vendorResponseSummaryRow-label' colspan='3'>Vendor Total:</td>"
            );
            vRow.AppendLine(
                $"  <td class='count' colspan='1'>{vendorTotal_responsesCount.ToString("0")}</td>"
            );
            vRow.AppendLine(
                $"  <td class='extension' colspan='1'>{vendorTotal_extensionPrice.ToString("0.00")}</td>"
            );
            vRow.AppendLine($"</tr>");
            vendorTotal_rowCount++;
            ...
            if (vendorTotal_responsesCount > 0)
            {
                ...
                t.AppendLine(
                    $"  <td class='vendorName' rowspan='{Math.Max(vendorTotal_rowCount, 1).ToString()}'>{v.VendorName}</td>"
                );
        ...
        t.AppendLine(
            $"  <td class='count' colspan='1'>{bidTotal_responsesCount.ToString("0")}</td>"
        );
        t.AppendLine(
            $"  <td class='extension' colspan='1'>{bidTotal_extensionPrice.ToString("0.00")}</td>"
        );
        t.AppendLine($"</tr>");
