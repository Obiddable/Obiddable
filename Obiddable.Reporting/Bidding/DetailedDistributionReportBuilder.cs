...
public class DetailedDistributionReportBuilder : BidHtmlReportBuilder
{
    ...
    {
        return _electionsRepo
            .GetItems_Responded_Elected_ByBid(Bid.Id)
    ...
    private string GenerateTable(List<Item> items)
    {
        ...
        foreach (var category in GetItemCategories(items))
        {
            ...
            {
                sb.AppendLine(
                    $"     <tr><td class='category-row' colspan='7'>{category.Key}</td></tr>"
                );
            ...
            foreach (Item i in GetCategoriesSortedItems(category))
            {
                ...
                List<RequestItem> requestItems = _requestingRepo
                    .GetRequestItems_ByItem(i.Id)
                    .OrderBy(x => x.Item.Code)
                    .ToList();

                sb.AppendLine($"     <tr>");
                sb.AppendLine(
                    $"         <td class='itemCode' rowspan='{Math.Max(requestItems.Count + 1, 1)}'>{i.FormattedCode}</td>"
                );
                sb.AppendLine(
                    $"         <td class='itemDescription' rowspan='{Math.Max(requestItems.Count + 1, 1)}'>{i.Description.Trim().Replace("\r\n", "<br />")}"
                        + $"{(responseItem.IsAlternate ? $"<br />(Alternate: {responseItem.Code} -- {responseItem.AlternateDescription?.Replace("\r\n", "<br />")})" : "")}"
                        + $"</td>"
                );
                sb.AppendLine(
                    $"         <td class='vendorName' rowspan='{Math.Max(requestItems.Count + 1, 1)}'><span class='clip'>{responseItem.VendorResponse.VendorName}</span></td>"
                );

                decimal total_itemExtensionPrice = 0;

                for (
                    int requestItemIndex = 0;
                    requestItemIndex < requestItems.Count;
                    requestItemIndex++
                )
                {
                    ...
                    string rowClass =
                        requestItemIndex == requestItems.Count - 1
                            ? " lastRequestorRow "
                            : "RequestorRow";
                    ...
                    sb.AppendLine(
                        $"         <td  class='requestorName'><span class='clip'>{requestItem.Request.Requestor.Name}</span></td>"
                    );
                    sb.AppendLine(
                        $"         <td  class='quantity'>{requestItem.Quantity.ToString("0.00")}</td>"
                    );
                    sb.AppendLine($"         <td class='unit'>{i.Unit}</td>");
                    sb.AppendLine(
                        $"         <td class='extension'>{(responseItem.Price * requestItem.Quantity).ToString("0.00")}</td>"
                    );
                    sb.AppendLine($"     </tr>");
                ...
                if (requestItems.Count > 0)
                {
                    ...
                    sb.AppendLine(
                        $"             <th class='extension' colspan='5'>{total_itemExtensionPrice.ToString("0.00")}</th>"
                    );
                    sb.AppendLine($"         </tr>");
                }
    ...
    private static IOrderedEnumerable<Item> GetCategoriesSortedItems(
        IGrouping<string, Item> category
    )
    {
    ...
    {
        return items.OrderBy(x => x.Category).GroupBy(x => x.Category).ToList();
    }
