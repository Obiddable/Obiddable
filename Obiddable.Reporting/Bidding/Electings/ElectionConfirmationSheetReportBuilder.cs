...
public class ElectionConfirmationSheetReportBuilder : BidHtmlReportBuilder
{
    ...
    private readonly int _safeLinesPerPage = 57;
    private readonly int _linesPerPage = 60;
    ...
    public ElectionConfirmationSheetReportBuilder()
        : base() { }
    ...
    public override string GenerateTableData(Bid Bid)
    {
        ...
        List<ResponseItem> responseItems = _respondingRepo
            .GetResponseItems_ByBid(Bid.Id)
            .OrderBy(x => x.Item.Code)
            .ToList();
        List<Item> items = _requestingRepo
            .GetItems_Requested_ByBid(Bid.Id)
            .Where(x => responseItems.Any(y => y.Item.Id == x.Id))
            .OrderBy(x => x.Code)
            .ToList();
        ...
        foreach (var category in items.OrderBy(x => x.Category).GroupBy(x => x.Category).ToList())
        {
            ...
            if (category.Key != "")
            {
                ...
                CheckAheadForPageBreak(t, _linesPerPage / 2);
                PrintSpacerRow(t);
            }

            foreach (Item i in categoryItems)
            {
                List<ResponseItem> itemResponses = responseItems
                    .Where(x => x.Item.Id == i.Id)
                    .OrderBy(x => x.GetExtendedPrice(_requestingRepo))
                    .ToList();

                int rowspan = Math.Max(
                    itemResponses.Count + itemResponses.Where(x => x.IsAlternate).Count(),
                    1
                );
                string[] descriptionLines = SplitStringToLines(
                    i.Description.Trim().Replace("\r\n", "<br />")
                );

                t.AppendLine($"<tr class='responseRow'>");
                t.AppendLine(
                    $"  <td class='itemCode' rowspan='{rowspan}'><span class='clip'>{i.GetFormattedCode()}</span></td>"
                );
                t.AppendLine(
                    $"  <td class='itemDescription' rowspan='{rowspan}'>{string.Join("<br />", descriptionLines)}</td>"
                );
                ...
                for (int responseIndex = 0; responseIndex < itemResponses.Count; responseIndex++)
                {
                    ...
                    t.AppendLine(
                        $"     <td class='{electedClass} vendorName ' ><span class='clip'>{ri.VendorResponse.VendorName}</span></td>"
                    );
                    t.AppendLine(
                        $"     <td class='{electedClass} partNumber ' ><span class='clip'>{ri.Code}</span></td>"
                    );
                    t.AppendLine(
                        $"     <td class='{electedClass} quantityResponded' >{ri.Get_Quantity(_requestingRepo).ToString("0.00")}</td>"
                    );
                    t.AppendLine(
                        $"     <td class='{electedClass} alt {(ri.IsAlternate ? "bold" : "")}'  ><span class='clip'>{(ri.IsAlternate ? "*" : "")}</span></td>"
                    );
                    t.AppendLine(
                        $"     <td class='{electedClass} unit'  ><span class='clip'>{(ri.IsAlternate ? ri.AlternateUnit : ri.Item.Unit)}</span></td>"
                    );
                    t.AppendLine(
                        $"     <td class='{electedClass} unitPrice' >{ri.Price.ToString("0.0000")}</td>"
                    );
                    t.AppendLine(
                        $"     <td class='{electedClass} extendedPrice' >{ri.GetExtendedPrice(_requestingRepo).ToString("$0.00")}</td>"
                    );
                    ...
                    if (ri.IsAlternate && ri.AlternateDescription?.Trim() != "")
                    {
                        t.AppendLine($"<tr>");
                        t.AppendLine(
                            $"  <td colspan='8' class='{electedClass} alternateDescription'><b>*</b>{ri.AlternateDescription?.Trim().Replace("\r\n", "<br />")}</td>"
                        );
                        t.AppendLine($"</tr>");
                    }
                }
    ...
    private void PrintCoverPage(StringBuilder t)
    {
        ...
        t.AppendLine(
            $"       The lowest vendor response is reflected in bold print for each item.                                                                           "
        );
        t.AppendLine(
            $"                                                                                                                                                      "
        );
        t.AppendLine(
            $"       For EVERY item, place a check mark in the box provided to the right of the vendor you are                                                      "
        );
        t.AppendLine(
            $"       selecting including if you are selecting the highlighted lowest vendor response.                                                               "
        );
        t.AppendLine(
            $"                                                                                                                                                      "
        );
        ...
        t.AppendLine(
            $"       -   Please note that if you are selecting a vendor other than the highlighted lowest vendor   -                                                "
        );
        t.AppendLine(
            $"       -   response, you MUST provide a detailed justification under the last vendor response in     -                                                "
        );
        t.AppendLine(
            $"       -   the area marked 'Reason for Change'.                                                      -                                                "
        );
        t.AppendLine(
            $"       -                                                                                             -                                                "
        );
    ...
    private void PrintFirstPageNumber(StringBuilder t)
    {
        t.Append("<pre>");
        t.AppendLine(
            $"Name:_______________________________                                                                                                          Page  {_pageNumber++.ToString("00")}"
        );
        t.AppendLine("");
        t.Append("</pre>");
    }
    ...
    {
        t.AppendLine(
            $"<tr><td class='category-row' colspan='{_columnCount}'>{categoryKey}</td></tr>"
        );
        AddLines(t, 3);
    }
    ...
    private void PrintTotalsRow(StringBuilder t, int totalCount_Items)
    {
        ...
        t.AppendLine(
            $"             <th colspan='{_columnCount}'>Total Items: {totalCount_Items.ToString("0")}</th>"
        );
        t.AppendLine($"         </tr>");
    }
    ...
    {
        t.AppendLine(
            $"<tr class='reasonForChange'><td colspan='{_columnCount}'>Reason For Change:</td></tr>"
        );
    }
    ...
    {
        t.AppendLine(
            $"<tr class='electionReason'><td colspan='{_columnCount}9'><span class='clip'>Election Reason: {electionReason}</span></td></tr>"
        );
    }
    #endregion
    ...
    {
        t.AppendLine($"<tr><td class='spacerRow' colspan='{_columnCount}'>&nbsp;</td></tr>");
        AddLines(t, 1);
    }
    ...
    private void PageBreak(StringBuilder t)
    {
        ...
        t.AppendLine(
            $"Name: ______________________________                                                                                                          Page  {_pageNumber++.ToString("00")}"
        );
        t.AppendLine("");
        t.Append("</pre>");
        _lines = 2;
    ...
    {
        if (_lines + lines > _safeLinesPerPage)
        {
            EndTable(t);
            PageBreak(t);
            StartTable(t);
