...
public abstract class HtmlReportBuilder<TObject> : IReportBuilder<TObject>
{
    ...
    public virtual IReportFile? BuildReport(TObject @object)
    {
        ...
        output.FileName =
            $"{output.TimeStamp.ToString("yyyy-MM-dd-HH-mm")}--{Title.ToLower().Replace(' ', '-')}--{FileNameSubtitle}.html";
        output.Data = CreateReport(
            Title,
            Subtitle,
    ...
    {
        sb.AppendLine(
            $"        <input type='button' onclick='window.print();' value='Print Report' class='noprint' />"
        );
    }

    private static void HeaderDiv(
        StringBuilder sb,
        string reportTitle,
    ...
    {
        sb.AppendLine(
            $"        <div class='eor'>End of Report {generationDate.ToString("yyyy-MM-dd-HH-mm")}</div>"
        );
    }
    ...
    private static string LoadStyle(string styleFileName)
    {
        ...
        using (
            StreamReader streamReader = new StreamReader(
                a.GetManifestResourceStream($"Obiddable.Reporting.Resources.Styles.{styleFileName}")
            )
        )
        {
            return streamReader
    ...
    public static string[] SplitStringToLines(string desc)
    {
        ...
        List<string> descriptionLines = desc.Split(
                new string[] { "<br />" },
