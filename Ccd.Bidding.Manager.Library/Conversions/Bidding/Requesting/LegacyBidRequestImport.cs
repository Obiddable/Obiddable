...
{
   public class LegacyBidRequestImport
   {
      public const int FIRST_ITEM_ROW = 9;
      ...
      {
         for (int row = FIRST_ITEM_ROW; ws.Cells[row, 1].Value != null; row++)
         {
            ...
            if (ws.Cells[row, 1].Value.ToString().Trim() == "*******")
            {
               err.AppendLine($"line skip: legacy header line ( line:{row} )");
               continue;
            }
