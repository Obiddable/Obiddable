...
namespace Obiddable.Library.EF.Responding;

public static class ResponseItemsValidations
{
    public static void Validate_AddResponseItem_ToVendorResponse(
        this Dbc dbc,
        ResponseItem obj,
        int vendorResponseId
    )
    {
    ...
    {
        var responseItem = dbc.ResponseItems
            .AsNoTracking()
            .Include(x => x.Item)
            .Single(x => x.Id == responseItemId);
    }

    public static void Validate_DeleteResponseItems_ByVendorResponse(
        this Dbc dbc,
        int vendorResponseId
    )
    {
        // validate each responseitem in vendor response
        var responseItems = dbc.ResponseItems
            .AsNoTracking()
